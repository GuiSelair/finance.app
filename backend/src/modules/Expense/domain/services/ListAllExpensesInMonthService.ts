...
class ListAllExpensesInMonthService {
  ...
  public async execute({ month, year, userId }: IListAllExpensesInMonth): Promise<ExpenseMonth[]> {
    if (month < 0 || month > 12)
    ...
    const expenses = await this.expensesMonthRepository.findByMonthAndYear(month, year, userId);
