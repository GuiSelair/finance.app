...
class FakeExpensesRepository implements IExpensesInMonthRepository {
  ...
  public async create(expenseMonthList: ICreateExpenseInMonth[]): Promise<ExpenseInMonth[]> {
  ...
  public async findByMonthAndYear(month: number, year: number): Promise<ExpenseInMonth[]> {
    return this.repository.filter(expense => expense.month === month && expense.year === year);
  }
}
