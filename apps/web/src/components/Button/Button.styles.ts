...
export const ButtonContainer = styled.button.withConfig({
	...
	${({ variant }: { variant?: keyof typeof BUTTON_VARIANT_MAPPER }) =>
		BUTTON_VARIANT_MAPPER[variant ?? 'solid']};
	${({ size }: { size?: keyof typeof BUTTON_SIZE_MAPPER }) =>
		BUTTON_SIZE_MAPPER[size ?? 'md']};
